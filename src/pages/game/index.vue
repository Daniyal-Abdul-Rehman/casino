<template>
  <q-layout>
    <q-page-container class="bg-green-800 min-h-screen">
      <Header />
      <div>
        <div class="slot-machine">
          <div
            class="slot"
            v-for="slot in this.allSlots"
            ref="slots"
            v-bind:key="slot.id"
          >
            <div class="slot-viewport" v-bind:style="[slotViewport]">
              <div class="slot-row">
                ...
                <div
                  class="slot-item slot-item-clone"
                  v-bind:style="[slotItem]"
                  :class="{ [opt]: true, active: isActiveSymbol(opt) }"
        ...
        <span v-if="this.currentBalance > 0">
          <div class="flex justify-center">
            <div class="lg:w-1/2 w-full px-5">
              ...
              <div
                class="bg-white text-green-800 text-lg font-bold rounded-md p-2 mb-4"
              >
                <div>Total credit balance</div>
                <div class="text-2xl">{{ this.currentBalance }} USD</div>
              </div>
              ...
              <div class="my-4">
                <div
                  ...
                  v-if="this.highlight.every((item) => item.active === true)"
                >
                  ...
                  >
                    <div class="rounded-full bg-white">
                      <img
                        ...
                        alt="Spinner"
                        class="rounded-full border-4 border-yellow-500"
                      />
                    </div>
                    <span
                      class="absolute inset-0 flex justify-center items-center text-yellow-600 font-bold text-lg"
                      >START</span
                    >
                  </div>
                </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <!-- Total Bet -->
                <div
                  class="flex items-center justify-between bg-white text-green-800 rounded-md p-2"
                >
                  <q-btn
                    flat
                    class="text-xl font-bold shadow-md border border-green-800"
                    @click="decrementBet"
                  >
                    -
                  </q-btn>
                  <div class="text-lg">{{ bet }} USD</div>
                  <q-btn
                    flat
                    @click="incrementBet"
                    class="text-xl font-bold shadow-md border border-green-800"
                ...
                >
                  <q-btn
                    flat
                    class="text-xl font-bold shadow-md border border-green-800"
                    @click="decrementPaylines"
                  >
                    -
                  </q-btn>
                  <div class="text-lg">{{ activePaylines }}</div>
                  <q-btn
                    flat
                    @click="incrementPaylines"
                    class="text-xl font-bold shadow-md border border-green-800"
                  >
                    +
                  </q-btn>
        ...
        <span v-else>
          <div
            class="bg-white text-green-800 text-lg font-bold rounded-md p-2 mb-4"
          >
            <div>Deposit Money in your account to play</div>
          </div>
        </span>
        ...
        <div class="app-details">
          ...
          <div>
            ...
            <a
              ...
              @click.prevent="changeBallance(10, 'add button')"
              >Add 10 <span class="coin tiny"></span
            ></a>
            <a
              class="btn small light"
              @click.prevent="changeBallance(-10, 'spent button')"
              >Spent 10 <span class="coin tiny"></span
            ></a>
            <br /><br /> -->
            <!-- <h3>Custom result</h3>
            <div class="debug">
              <div>
                ...
                <select v-model="debug.line[0]">
                  ...
                  <option v-for="n in 3" :value="[n]" v-bind:key="n">
                    {{ n }}
                  </option>
                </select>
              </div>
            ...
            <div v-for="bc in this.balanceRecords.records" v-bind:key="bc.id">
              <span class="coin tiny"></span> {{ bc.changes }}
...
const slotMachine = {
  ...
  methods: {
    incrementBet() {
      this.bet += 10; // increment bet by 5
    },
    decrementBet() {
      if (this.bet > 5) {
        this.bet -= 10; // decrement bet by 5
      }
    },
    incrementPaylines() {
      if (this.activePaylines < 5) {
        this.activePaylines += 1;
      } // increment activePaylines
    },
    decrementPaylines() {
      if (this.activePaylines > 1) {
        this.activePaylines -= 1; // decrement activePaylines
      }
    },
    ...
    start() {
      ...
      this.$store.dispatch("changeBalance", {
        diff: -(this.bet * this.activePaylines),
        type: "press button",
      });
      // define animation
      ...
      this.reels = this.allSlots.map((data, i) => {
        ...
        const reel = {
          ...
          finalPos:
            choice * slotSize -
            (halhLine ? slotSize / 2 : 0) +
            data.items.length * slotSize,
          startOffset:
            baseDuration + Math.random() * timeOffest + i * timeOffest,
    ...
    animate(timestamp) {
      ...
      this.reels.forEach((reel) => {
        ...
        const power = 3;
        const offset =
          (Math.pow(timeRemaining, power) / Math.pow(reel.duration, power)) *
          reel.startOffset;
    ...
    checkPayTable(reels) {
      ...
      result.forEach((resultLine, index) => {
        ...
        const uniqueSymbols = resultLine.filter(
          (val, index, self) => self.indexOf(val) === index
        ).length; // unique symbols in line
        const isCombination = uniqueSymbols > 1; // if symbols are different
        if (fullLine) {
          // only then check line rewards
          payLines.forEach((payout) => {
            const matches = resultLine.filter(
              (symbol) => payout.items.indexOf(symbol) !== -1
            ); // filter only matching symbols
            ...
            if (matches.length === 3) {
              if (
                (payout.count === "combination" && isCombination) ||
                payout.count === 3
              ) {
...
.slot-machine {
  ...
  .slot {
    ...
    .slot-viewport {
      ...
      .slot-item.bar {
        background-image: url("/Gaming_Pictures/g1.jpeg");
      }
      .slot-item.bar2 {
        background-image: url("/Gaming_Pictures/g2.jpeg");
      }
      .slot-item.bar3 {
        background-image: url("/Gaming_Pictures/g3.jpeg");
      }
      .slot-item.cherry {
        background-image: url("/Gaming_Pictures/g4.jpeg");
      }
      .slot-item.seven {
        background-image: url("/Gaming_Pictures/g5.jpeg");
      }
    }
  }
}
